...
// // Strict type for Uniforms to avoid 'any'
// type UniformValue = 
//   | number 
//   | boolean 
//   | THREE.Vector2 
...
// interface IUniform<T = UniformValue> {
//   value: T;
// }

// type ShaderUniforms = { [key: string]: IUniform };
...
// // --- Singleton Geometry (Optimization) ---
// // Reusing this prevents creating new buffers for every shader pass
...
// }: LiquidEtherProps): React.ReactElement {
//   const mountRef = useRef<HTMLDivElement | null>(null);
//   // We use a generic ref to hold the manager instance
...
//     // --- Helper Classes defined inside scope to capture instance specifics ---
...
//       init(container: HTMLElement) {
//         this.container = container;
//         // Bind to window for smoother drag outside container, but container for entry
...
//       updatePass(targetOverride?: THREE.WebGLRenderTarget) {
//         if (!Common.renderer) return;
        
//         const target = targetOverride || this.props.output || null;
...
//         // Do not dispose singleton geometry
//       }
//     }
...
//         if (dt !== undefined) this.uniforms.dt.value = dt;
//         if (isBounce !== undefined && this.line) this.line.visible = isBounce;
//         if (BFECC !== undefined) this.uniforms.isBFECC.value = BFECC;
//         this.updatePass();
//       }
//     }
...
//       constructor(cellScale: THREE.Vector2, cursor_size: number, dst: THREE.WebGLRenderTarget) {
//         super({
//           material: {
//              // Will be overridden by mouseMesh material but required by base class
//             vertexShader: mouse_vert,
//             fragmentShader: externalForce_frag,
//             uniforms: {}
//           },
...
//         // The base class creates a full screen plane, but ExternalForce uses a custom small plane
...
//       update({ cursor_size, mouse_force, cellScale }: { cursor_size: number, mouse_force: number, cellScale: THREE.Vector2 }) {
...
//               velocity: { value: src },
//               velocity_new: { value: dst_.texture },
//               v: { value: viscousVal },
//               px: { value: cellScale },
//               dt: { value: dtVal }
//             }
...
//           this.uniforms.velocity_new.value = fbo_in.texture;
//           this.updatePass(fbo_out);
//         }
        
//         return (iterations % 2 === 0) ? this.props.output1! : this.props.output0!;
//       }
//     }

//     class Divergence extends ShaderPass {
//       constructor(cellScale: THREE.Vector2, boundarySpace: THREE.Vector2, src: THREE.Texture, dst: THREE.WebGLRenderTarget, dtVal: number) {
//         super({
//           material: {
//             vertexShader: face_vert,
//             fragmentShader: divergence_frag,
...
//     class Poisson extends ShaderPass {
//       constructor(cellScale: THREE.Vector2, boundarySpace: THREE.Vector2, src: THREE.Texture, dst: THREE.WebGLRenderTarget, dst_: THREE.WebGLRenderTarget) {
//         super({
//           material: {
//             vertexShader: face_vert,
//             fragmentShader: poisson_frag,
//             uniforms: {
//               boundarySpace: { value: boundarySpace },
//               pressure: { value: dst_.texture },
//               divergence: { value: src },
//               px: { value: cellScale }
//             }
//           },
//           output: dst,
...
//             uniforms: {
//               boundarySpace: { value: boundarySpace },
//               pressure: { value: src_p },
//               velocity: { value: src_v },
//               px: { value: cellScale },
//               dt: { value: dtVal }
//             }
//           },
...
//         ['vel_0', 'vel_1', 'vel_viscous0', 'vel_viscous1', 'div', 'pressure_0', 'pressure_1'].forEach(name => {
//           this.fbos[name] = new THREE.WebGLRenderTarget(this.fboSize.x, this.fboSize.y, opts);
//         });
//       }

//       createShaderPass() {
//         this.advection = new Advection(this.cellScale, this.fboSize, this.options.dt, this.fbos.vel_0.texture, this.fbos.vel_1);
//         this.externalForce = new ExternalForce(this.cellScale, this.options.cursor_size, this.fbos.vel_1);
        
//         this.viscous = new Viscous(
//           this.cellScale, this.boundarySpace, this.options.viscous, 
//           this.fbos.vel_1.texture, this.fbos.vel_viscous1, this.fbos.vel_viscous0, this.options.dt
//         );
        
//         this.divergence = new Divergence(this.cellScale, this.boundarySpace, this.fbos.vel_viscous0.texture, this.fbos.div, this.options.dt);
//         this.poisson = new Poisson(this.cellScale, this.boundarySpace, this.fbos.div.texture, this.fbos.pressure_1, this.fbos.pressure_0);
        
//         this.pressure = new Pressure(
//           this.cellScale, this.boundarySpace, this.fbos.pressure_0.texture, this.fbos.vel_viscous0.texture, 
//           this.fbos.vel_0, this.options.dt
//         );
//       }
...
//       constructor(simOptions: SimOptions) {
...
//       constructor(container: HTMLElement, initOpts: SimOptions) {
...
//         this.output = new Output(initOpts);
...
//         if(this.$wrapper && Common.renderer) {
...
//         this.output.dispose();
//         Common.dispose();
//         if (Common.renderer && Common.renderer.domElement && Common.renderer.domElement.parentNode) {
...
//     const initialSimOpts: SimOptions = {
//         iterations_poisson: iterationsPoisson,
//         iterations_viscous: iterationsViscous,
//         mouse_force: mouseForce,
...
//     const manager = new WebGLManager(container, initialSimOpts);
...
//     // We only run this EFFECT on MOUNT/UNMOUNT. 
//     // Updates are handled by the second useEffect below.
//     // eslint-disable-next-line react-hooks/exhaustive-deps
//   }, [colors]); // Re-init only if colors change deep enough to require texture rebuild


//   // --- Prop Updates ---
//   // This separate effect pushes prop changes to the running simulation without destroying it
//   useEffect(() => {
//     const manager = managerRef.current;
//     if (!manager) return;

//     const sim = manager.output.simulation;
//     const prevRes = sim.options.resolution;

//     // Update Props
//     sim.options.mouse_force = mouseForce;
//     sim.options.cursor_size = cursorSize;
//     sim.options.isViscous = isViscous;
//     sim.options.viscous = viscous;
//     sim.options.iterations_viscous = iterationsViscous;
//     sim.options.iterations_poisson = iterationsPoisson;
//     sim.options.dt = dt;
//     sim.options.BFECC = BFECC;
//     sim.options.resolution = resolution;
//     sim.options.isBounce = isBounce;

//     // Update AutoDriver
//     if (manager.autoDriver) {
//         manager.autoDriver.enabled = autoDemo;
//         manager.autoDriver.speed = autoSpeed;
//         manager.autoDriver.resumeDelay = autoResumeDelay;
//         manager.autoDriver.rampDurationMs = autoRampDuration * 1000;
        
//         // Update Mouse Driver props
//         const mouse = manager.autoDriver.mouse;
//         if(mouse) {
...
//     // Handle Resolution Change (requires FBO resize)
...
//   return <div ref={mountRef} className={`liquid-ether-container ${className}`} style={{ position: 'relative', overflow: 'hidden', ...style }} />;
// }
...
//       init(container: HTMLElement) {
//         this.container = container;
//         // DISABLED MOUSE INTERACTION as requested.
//         // The event listeners are commented out so user input is ignored.
//         // window.addEventListener('mousemove', this._onMouseMove);
//         // window.addEventListener('touchstart', this._onTouchStart, { passive: true });
//         // window.addEventListener('touchmove', this._onTouchMove, { passive: true });
//         // window.addEventListener('touchend', this._onTouchEnd);
...
//         if (!this.enabled) return;
//         const now = performance.now();
//         // Since mouse interaction is disabled, idle will always increase, keeping AutoDriver active
//         const idle = now - this.manager.lastUserInteraction;
...
//       constructor(options: SimOptions) {
//         this.options = options;
//         // FIX: Replaced 'any' with the specific type Record<string, THREE.WebGLRenderTarget>
//         this.fbos = {} as Record<string, THREE.WebGLRenderTarget>;
//         this.init();
//       }
...
//   // FIX 1: Replaced <any> with a specific interface for the manager methods
...
//       constructor(options: SimOptions) {
//         this.options = options;
//         // FIX 2: Replaced 'any' with the specific type Record<string, THREE.WebGLRenderTarget>
//         this.fbos = {} as Record<string, THREE.WebGLRenderTarget>;
//         this.init();
//       }
...
// --- Interfaces to replace 'any' in useRef ---

interface MouseControllerInterface {
  autoIntensity: number;
  takeoverDuration: number;
  ...
  update: () => void;
  forceStop?: () => void; // Optional depending on usage
}

interface AutoDriverInterface {
  enabled: boolean;
  speed: number;
  resumeDelay: number;
  rampDurationMs: number;
...
export default function LiquidEther({
  ...
  // FIX: Typed 'autoDriver' and 'output' with interfaces instead of 'any'
  const managerRef = useRef<{ 
    start: () => void; 
    pause: () => void; 
    dispose: () => void; 
    resize: () => void;
    autoDriver?: AutoDriverInterface;
  ...
  useEffect(() => {
    ...
    // MouseClass implements MouseControllerInterface implicitly
    class MouseClass implements MouseControllerInterface {
      mouseMoved = false;
      coords = new THREE.Vector2();
      coords_old = new THREE.Vector2();
      diff = new THREE.Vector2();
    ...
    // AutoDriver implements AutoDriverInterface implicitly
    class AutoDriver implements AutoDriverInterface {
      active = false;
      current = new THREE.Vector2(0, 0);
      target = new THREE.Vector2();
      lastTime = performance.now();
      ...
      // FIX: Changed 'private' to 'public' to allow interface access in props update
      constructor(
        public mouse: MouseClass,
        private manager: WebGLManager,
        opts: { enabled: boolean; speed: number; resumeDelay: number; rampDuration: number }
      ) {
        this.enabled = opts.enabled;
    ...
    // Simulation implements SimulationInterface implicitly
    ...
    // Output implements OutputInterface implicitly
  ...
  useEffect(() => {
    ...
    // Safety check if output or simulation is missing (though typed as present)
    if (!manager.output || !manager.output.simulation) return;
