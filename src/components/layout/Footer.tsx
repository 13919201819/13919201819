  ...
  return (
    <footer className="relative bg-black border-t border-gray-800/50">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="py-8 sm:py-12 lg:py-16 grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-8 sm:gap-10 lg:gap-12">
          
          {/* Brand Section - 2 cols on all screen sizes */}
          <div className="col-span-2 space-y-4 sm:space-y-6">
            <div className="flex items-center gap-2 sm:gap-3">
              {/* <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-gradient-to-br from-purple-600 to-violet-600 flex items-center justify-center p-2">
              ...
              <span className="text-xl sm:text-2xl font-bold text-white tracking-tight">MistrAI</span>
            </div>
            <p className="text-gray-400 text-sm sm:text-base leading-relaxed max-w-md">
              Transforming businesses with enterprise-grade AI products and solutions. Building the future of intelligent automation.
            </p>
            <div className="flex items-center gap-2 sm:gap-3">
              {socialLinks.map((social) => (
                <a 
                  key={social.name} 
                  href={social.href} 
                  className="p-2.5 sm:p-3 rounded-lg bg-gray-900/50 border border-gray-800 hover:border-purple-500/50 text-gray-400 hover:text-purple-400 transition-all duration-300 hover:scale-110"
            ...
            <ul className="space-y-2 sm:space-y-3">
              {quickLinks.map((link) => (
                <li key={link.name}>
                  <a 
                    href={link.href} 
                    className="text-gray-400 hover:text-purple-400 transition-colors inline-flex items-center gap-2 group text-sm sm:text-base"
                  >
                    <span>{link.name}</span>
                    <svg className="w-3 h-3 sm:w-4 sm:h-4 opacity-0 group-hover:opacity-100 transform -translate-x-2 group-hover:translate-x-0 transition-all" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="border-t border-gray-800/50 py-6 sm:py-8">
          <div className="flex flex-col md:grid md:grid-cols-3 items-center gap-4 sm:gap-6">
            ...
            {/* Developer Credit - Order 1 on mobile, 2 on desktop */}
